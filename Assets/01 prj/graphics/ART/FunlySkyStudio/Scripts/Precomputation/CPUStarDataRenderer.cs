...
namespace Funly.SkyStudio
{
  ...
  public class CPUStarDataRenderer : BaseStarDataRenderer
  {
    ...
    List<StarPoint> GenerateRandomStarsPoints(float starDensity, int imageWidth, int imageHeight)
    {
      int numStars = Mathf.FloorToInt((float)imageWidth * (float)imageHeight * Mathf.Clamp01(starDensity));
